...
type Config struct {
	...
	WorkerConcurrency int // FIX: was missing — worker.go references config.C.WorkerConcurrency
}
...
func Init() {
	...
	once.Do(func() {
		...
		cfg := &Config{
			...
			WorkerConcurrency: mustGetEnvInt("WORKER_CONCURRENCY", 5),
...
func validate(c *Config) {
	...
	if c.WorkerConcurrency <= 0 {
		fatal("WORKER_CONCURRENCY must be > 0")
	}
}
...
func logConfig(c *Config) {
	log.Printf(
		"event=config_loaded mode=%s worker=%s max_retry=%d max_queue=%d recovery_timeout=%s concurrency=%d",
		c.AppMode,
		c.WorkerID,
		c.MaxRetry,
		c.MaxQueueSize,
